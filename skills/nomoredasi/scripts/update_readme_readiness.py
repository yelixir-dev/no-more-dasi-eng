#!/usr/bin/env python3
"""Update the machine-maintained corpus coverage section in both READMEs."""
...
def readiness_section(records, majors, korean=False):
    if korean:
        heading = "## 분야 코퍼스 커버리지"
        alt = "분야 코퍼스 커버리지 차트"
        field_header, papers_header, score_header = "분야", "논문", "커버리지"
        full_list = f"세부 분야 포함 전체 목록: [{FIELDS_LINK}]({FIELDS_LINK})"
        note = "auto-updated by the delta cycle"
    else:
        heading = "## Field corpus coverage"
        alt = "Field corpus coverage chart"
        field_header, papers_header, score_header = "Field", "Papers", "Coverage"
...
def fields_html(records, majors):
    ...
    def table(items):
        ...
        return (
            "<table><tr><th>Field</th><th>Papers</th><th>Coverage</th></tr>"
            + rows
            + "</table>"
        )
    ...
    body = (
        ...
        + panel(
            "Notes",
            "method",
            "<p>Corpus coverage values are the 0-100 composite computed by "
    ...
    return page(
        "Field Corpus Coverage — Full List",
        hero(
            "NOMOREDASI · FIELD CORPUS COVERAGE",
            "Field corpus coverage — full list",
            "All Nature subjects tracked by the nomoredasi corpus, with per-field paper "
            "counts and corpus coverage values.",
            [
