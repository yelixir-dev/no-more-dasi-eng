...
GRID = "#e2dccf"    # warm, precise thin gridline
AXIS = "#28231f"    # tick + frame ink
TEXT = "#28231f"
MUTED = "#6d665e"
THRESH = "#8a5a00"
...
def render_html(history_path, out_path):
    ...
    for gy, label in THRESHOLDS:
        svg.append(
            ...
            f'stroke="#b57920" stroke-width="1.25" stroke-dasharray="6 5"/> '
            f'<text x="{ML + 8}" y="{y(gy) - 7:.1f}" font-size="10" fill="#8a5a00" '
    ...
    for gy in range(0, 101, 20):
        ...
        svg.append(f'<line x1="{ML}" y1="{y(gy):.1f}" x2="{ML - 5}" y2="{y(gy):.1f}" stroke="{AXIS}"/>')
        svg.append(f'<text x="{ML - 10}" y="{y(gy) + 4:.1f}" text-anchor="end" font-size="11" fill="{MUTED}" {attr}>{gy}</text>')

    step = nice_step(x_max)
    xv = 0
    while xv <= x_max:
        svg.append(f'<line x1="{x(xv):.1f}" y1="{MT}" x2="{x(xv):.1f}" y2="{MT + PH}" stroke="{GRID}"/>')
        svg.append(f'<line x1="{x(xv):.1f}" y1="{MT + PH}" x2="{x(xv):.1f}" y2="{MT + PH + 5}" stroke="{AXIS}"/>')
        svg.append(f'<text x="{x(xv):.1f}" y="{MT + PH + 22}" text-anchor="middle" font-size="11" fill="{MUTED}" {attr}>{xv}</text>')
        xv += step

    svg.append(f'<rect x="{ML}" y="{MT}" width="{PW}" height="{PH}" fill="none" stroke="{AXIS}" stroke-width="1.25"/>')
    ...
    for field, recs in sorted(series.items()):
        color = color_of.get(field, "#8a8177")
    ...
    extra_css = """
<style>
.section-note, .caption, .method-note { color: var(--muted); font-family: -apple-system, BlinkMacSystemFont, "Segoe UI", sans-serif; font-size: .84rem; }
.chart-frame { background: #fbf7f0; border: 1px solid var(--rule); padding: .5rem; }
.chart-frame svg { display: block; height: auto; max-width: 100%; }
.legend-heading { color: var(--rust); font-family: -apple-system, BlinkMacSystemFont, "Segoe UI", sans-serif; font-size: .73rem; font-weight: 800; letter-spacing: .1em; margin: 1.3rem 0 .45rem; text-transform: uppercase; }
...
.legend-list li { align-items: center; border-bottom: 1px solid var(--rule); display: grid; gap: .45rem; grid-template-columns: .75rem 1fr auto; font-family: -apple-system, BlinkMacSystemFont, "Segoe UI", sans-serif; font-size: .8rem; min-width: 0; padding: .32rem 0; }
...
.legend-list strong { color: var(--gold); font-variant-numeric: tabular-nums; }
.caption { margin: 1rem 0 0; }
.table-wrap { max-width: 100%; overflow-x: auto; }
table { font-variant-numeric: tabular-nums; min-width: 42rem; }
thead th { background: #eee6db; color: var(--ink); letter-spacing: .06em; text-transform: uppercase; }
tbody tr:nth-child(even) { background: #fbf7f0; }
.num { text-align: right; }
.score { color: var(--gold); font-weight: 700; }
.method-note { border-left: .25rem solid var(--gold); margin: 1.5rem 0 0; padding: .1rem 0 .1rem 1rem; }
...
"""
    ...
    hero_html = hero(
        ...
        meta_items=[
            ("History", escape(history_path.name)),
    ...
    chart_panel = panel(
        ...
        "paper counts keep their latest dated record.</p>",
    )
