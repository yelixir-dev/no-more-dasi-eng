...
def pie_svg(groups):
    ...
    for i, (subject, items) in enumerate(groups):
        ...
        parts.append(f'<text x="{legend_x + 14}" y="{ly}" font-size="11" fill="#17202a">{label} ({papers_label(len(items))})</text>')
    parts.append("</svg>")
    return "".join(parts)
...
def render_html(entries, as_of):
    ...
    parts = [
        ...
        f"{papers_label(total)} across {len(groups)} fields · {len(excluded)} excluded</div></div>",
        '<div class="card">',
        pie_svg(groups),
        "</div>",
        "<h2>Registers by field (click to expand)</h2>",
    ]
    for subject, items in groups:
        parts.append("<details>")
        parts.append(f"<summary><b>{escape(subject)} ({papers_label(len(items))})</b></summary>")
